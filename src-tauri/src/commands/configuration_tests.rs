...
#[test]
fn tunnel_secret_selection_is_provider_channel_and_source_specific() {
    ...
    profile.actions.tunnel_type = "none".into();
    let valid = TunnelSecretUpdate { key: "cloudflare_token".into(), value: "canary".into() };
    assert_eq!(tunnel_secret::validate(&profile, &valid).unwrap(), ServiceKind::Mcp);
    for key in ["oauth_client_secret", "actions_cloudflare_token", "frp_token"] {
        let bad = TunnelSecretUpdate { key: key.into(), value: "do-not-echo-canary".into() };
        let error = tunnel_secret::validate(&profile, &bad).unwrap_err().to_string();
        assert!(!error.contains(&bad.value));
    }
    for value in [String::new(), "\n".into(), "x".repeat(16385)] {
        assert!(tunnel_secret::validate(&profile, &TunnelSecretUpdate { key: valid.key.clone(), value }).is_err());
    }
    profile.tunnel.cloudflare_mode = "quick".into();
    assert!(tunnel_secret::validate(&profile, &valid).is_err());
    profile.tunnel.tunnel_type = "frp".into();
    profile.tunnel.frp_profile_id = "global-profile".into();
    assert!(tunnel_secret::selected_service(&profile, "frp_token").is_none());
    profile.tunnel.frp_profile_id.clear();
    assert_eq!(tunnel_secret::selected_service(&profile, "frp_token"), Some(ServiceKind::Mcp));
}

#[tokio::test]
async fn invalid_atomic_tunnel_credential_cannot_stop_or_modify_a_live_workspace() {
    if test_process::run_isolated("invalid_atomic_tunnel_credential_cannot_stop_or_modify_a_live_workspace") { return; }
    let f = Fixture::new();
    runtime::start_mcp_service(&f.state, &f.profile.id).await.unwrap();
    let mut candidate = f.profile.clone(); candidate.name = "must-not-commit".into();
    let invalid = TunnelSecretUpdate { key: "oauth_token_secret".into(), value: "must-not-store".into() };
    assert!(update_with_tunnel_secret(&f.state, candidate, Some(invalid)).await.is_err());
    assert!(f.state.with_runtime(|runtime| Ok(runtime.is_running(&f.profile.id, ServiceKind::Mcp))).unwrap());
    let saved = f.state.with_workspaces(|store| Ok(store.get(&f.profile.id).unwrap().name.clone())).unwrap();
    assert_eq!(saved, f.profile.name);
    assert_eq!(f.client.get(format!("{}/mcp", f.base())).send().await.unwrap().status(), 200);
}

#[tokio::test]
async fn stopped_atomic_tunnel_save_updates_secret_without_starting_services() {
    if test_process::run_isolated("stopped_atomic_tunnel_save_updates_secret_without_starting_services") { return; }
    let mut f = Fixture::new();
    f.profile.actions.tunnel_type = "cloudflare".into();
    f.profile.actions.cloudflare_mode = "named".into();
    f.profile.actions.public_url = "https://named-fixture.example".into();
    f.profile.actions.cloudflare_token = "legacy-inline-canary".into();
    let secret = TunnelSecretUpdate { key: "actions_cloudflare_token".into(), value: "new-snapshot-canary".into() };
    update_with_tunnel_secret(&f.state, f.profile.clone(), Some(secret)).await.unwrap();
    f.state.with_data(|store| {
        assert!(store.get(&f.profile.id).unwrap().actions.cloudflare_token.is_empty());
        assert_eq!(store.get_workspace_secret(&f.profile.id, "actions_cloudflare_token")?.as_deref(), Some("new-snapshot-canary"));
        Ok(())
    }).unwrap();
    for kind in [ServiceKind::Mcp, ServiceKind::Actions] {
        assert!(!f.state.with_runtime(|runtime| Ok(runtime.is_running(&f.profile.id, kind))).unwrap());
    }
}

#[tokio::test]
async fn strict_application_reports_connector_failure_without_restoring_old_auth() {
    if test_process::run_isolated("strict_application_reports_connector_failure_without_restoring_old_auth") { return; }
    let mut f = Fixture::new();
    // A deliberately incomplete legacy FRP profile deterministically fails before network I/O.
    f.profile.tunnel.tunnel_type = "frp".into();
    f.profile.tunnel.frp_server.clear();
    f.profile.tunnel.frp_profile_id.clear();
    f.profile.auth.auth_type = "oauth".into();
    f.state.with_workspaces(|store| store.update(f.profile.clone())).unwrap();
    let result = runtime::start_mcp_service_checked(&f.state, &f.profile.id, true).await;
    assert!(result.is_err());
    assert!(f.state.with_runtime(|runtime| Ok(runtime.is_running(&f.profile.id, ServiceKind::Mcp))).unwrap());
    assert_eq!(f.client.post(format!("{}/mcp", f.base())).json(&json!({"jsonrpc":"2.0","id":1,"method":"initialize"}))
