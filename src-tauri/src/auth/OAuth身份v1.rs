...
pub(crate) fn verify(token: &str, secret: &str, origin: &str) -> Option<VerifiedPrincipal> {
    if secret.is_empty() || token.len() > 8192 { return None; }
    let origin = origin.trim_end_matches('/');
    let mut v = Validation::new(Algorithm::HS256);
    v.set_issuer(&[origin]); v.set_audience(&[origin]); v.leeway = 0; v.validate_nbf = true;
...
mod tests {
    ...
    fn rejects_wrong_issuer_audience_scope_identity_time_and_signature() {
        ...
        assert!(verify(&token,"test-secret","https://host").is_some());
        assert!(verify(&token,"wrong-secret","https://host").is_none());
        assert!(verify(&token,"test-secret","https://other").is_none());
