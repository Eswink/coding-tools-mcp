...
impl ExecSpec {
    ...
    pub fn with_env(mut self, key: impl Into<String>, value: impl Into<String>) -> Result<Self, ExecError> {
    ...
    fn validate(&self) -> Result<(), ExecError> {
        if self.argv.is_empty() || self.argv.len() > MAX_ARGC {
            return Err(ExecError::new(ExecErrorKind::InvalidSpec, "invalid argv length"));
        }
        ...
        for token in &self.argv {
            ...
            {
                return Err(ExecError::new(ExecErrorKind::InvalidSpec, "invalid argv token"));
            }
        ...
        if command_bytes > MAX_COMMAND_BYTES {
            return Err(ExecError::new(ExecErrorKind::InvalidSpec, "argv is too large"));
        }
        if self.env.len() > MAX_ENV_VARS {
            return Err(ExecError::new(ExecErrorKind::InvalidSpec, "too many environment variables"));
        }
        ...
        for (key, value) in &self.env {
            ...
            env_bytes = env_bytes
                ...
                .ok_or_else(|| ExecError::new(ExecErrorKind::InvalidSpec, "environment size overflow"))?;
        }
        ...
        if self.stdin.len() > MAX_STDIN_BYTES {
            return Err(ExecError::new(ExecErrorKind::InvalidSpec, "stdin is too large"));
        }
        if self.timeout.is_zero() || self.timeout > MAX_TIMEOUT {
            return Err(ExecError::new(ExecErrorKind::InvalidSpec, "invalid timeout"));
        }
...
impl ProcessManager {
    ...
    pub async fn start(&self, spec: ExecSpec) -> Result<ProcessSession, ExecError> {
        ...
        let cwd = std::fs::canonicalize(&spec.cwd)
            .map_err(|_| ExecError::new(ExecErrorKind::InvalidSpec, "working directory unavailable"))?;
        if !cwd.is_dir() {
            return Err(ExecError::new(
                ExecErrorKind::InvalidSpec,
        ...
        let permit = self
            ...
            .clone()
            .try_acquire_owned()
            .map_err(|_| ExecError::new(ExecErrorKind::Capacity, "process capacity exhausted"))?;
