    ...
    fn validate_examples(&self) -> Result<(), PolicyError> {
        for argv in &self.must_match {
            let command = Command::new(argv.clone()).map_err(|_| {
                PolicyError::new(PolicyErrorKind::InvalidExample, "invalid positive rule example")
            })?;
        ...
        for argv in &self.must_not_match {
            let command = Command::new(argv.clone()).map_err(|_| {
                PolicyError::new(PolicyErrorKind::InvalidExample, "invalid negative rule example")
            })?;
            if self.matches(command.argv()) {
                return Err(PolicyError::new(
                    PolicyErrorKind::InvalidExample,
    ...
    pub fn authorize(
        ...
        match self.evaluate(command).decision {
            ...
            Some(ExecDecision::Prompt) => {
                if approval.is_some_and(|approval| {
                    approval.matches(command, call, verified, now_unix_ms)
                }) {
...
mod tests {
    ...
    fn alternatives_and_prefix_matching_are_deterministic() {
        ...
        assert_eq!(
            policy.evaluate(&command(&["git", "status", "--short"])).decision,
    ...
    fn strictest_matching_decision_wins() {
        ...
        .unwrap();
        assert_eq!(
            policy.evaluate(&command(&["git", "push", "--force"])).decision,
    ...
    fn no_match_is_explicit_not_allow() {
        let policy = ExecPolicy::new(vec![exact(&["git"], ExecDecision::Allow)], vec![], false)
            .unwrap();
    ...
    fn host_executable_fallback_is_opt_in_and_path_constrained() {
        ...
        assert_eq!(
            disabled.evaluate(&command(&["/usr/bin/git", "status"])).decision,
            None
        );
        ...
        assert_eq!(
            enabled.evaluate(&command(&["/opt/bin/git", "status"])).decision,
            None
        );
    }
