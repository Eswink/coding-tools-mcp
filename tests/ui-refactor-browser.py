...
try:
    with sync_playwright() as p:
        ...
        if MODE=='candidate':
            ...
            # Save through the real form handler, not by mutating its dirty state.
            fresh()
            client=page.get_by_role('textbox',name='OAuth 客户端 ID',exact=True)
            ...
            page.get_by_role('button',name='Actions 服务',exact=False).click()
            expect(page.get_by_role('button',name='Actions 服务',exact=False)).to_have_attribute('aria-pressed','true')
            assert not page.evaluate("window.__UI_FIXTURE__.state.calls.some(c=>c.command==='plugin:dialog|message'&&c.args.title==='确认切换面板')")
            done('successful actual form save clears navigation warning without granting permissions')
            fresh()
            client=page.get_by_role('textbox',name='OAuth 客户端 ID',exact=True)
            original=client.input_value();client.fill('temporary-synthetic-edit');client.fill(original)
            page.get_by_role('tab',name='日志',exact=True).click()
            expect(page.get_by_role('tab',name='日志',exact=True)).to_have_attribute('aria-selected','true')
            assert not page.evaluate("window.__UI_FIXTURE__.state.calls.some(c=>c.command==='plugin:dialog|message'&&c.args.title==='确认切换面板')")
            done('reverted input is clean and does not produce a stale leave confirmation')
