...
$installRoot = Join-Path $env:RUNNER_TEMP ('exclusive-install-' + [Guid]::NewGuid().ToString('N'))
...
$fixture = Join-Path $env:RUNNER_TEMP ('exclusive-config-' + [Guid]::NewGuid().ToString('N'))
...
$record = [ordered]@{
    ...
    scenario='exclusive-refresh-v1'; kind='nsis'; package=@{name=$package.Name; size=$package.Length; sha256=(Get-FileHash -LiteralPath $target -Algorithm SHA256).Hash.ToLowerInvariant()};
...
$record | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $OutputDirectory 'exclusive-package.json') -Encoding utf8
