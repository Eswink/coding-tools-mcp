...
Windows v0.2.4 的用户本机验收已由用户于 2026-09-09 确认；本版不覆盖该 Release、不移动旧标签。v0.2.6 同时交付 Ubuntu 与 Windows 安装包，两端来自同一提交，并分别通过原生门禁。实际发布状态以本次Actions及Release附件为准。
...
在 Release 下载 `MCP_0.2.6_amd64.deb` 和 `SHA256SUMS_v0.2.6.txt`。先进入下载目录，核对所下载文件的 SHA-256，再安装：

```bash
sha256sum MCP_0.2.6_amd64.deb
# 将结果与 SHA256SUMS_v0.2.6.txt 对应行逐字核对。
sudo apt update
...
如安装时系统显示的包名不同，以 `dpkg-deb -f MCP_0.2.6_amd64.deb Package` 输出为准。不要手动删除密钥环来“清空配置”。

## 使用 AppImage

下载 `MCP_0.2.6_amd64.AppImage` 并核对摘要：

```bash
sha256sum MCP_0.2.6_amd64.AppImage
chmod +x MCP_0.2.6_amd64.AppImage
./MCP_0.2.6_amd64.AppImage
```
...
只有全部门禁通过，才公开 **预发布** Release。附件按顺序上传并核对大小/摘要；公开后再匿名下载复核。`Ubuntu-evidence_v0.2.6.zip` 保存来源、原生结果和截图；公开下载回执另保存在 Actions 工件中。工作流定义不是执行通过证明，结果以实际运行和附件为准。源码未提供商业软件签名，下载时核对 Release 的 SHA-256。
