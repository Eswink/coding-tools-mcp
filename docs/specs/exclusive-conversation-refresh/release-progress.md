...
## Round 20B — Exact-source pre-release closure

Add a separate exclusive-release workflow and publisher, not an override of the legacy eight-stage release. Only the dedicated release/exclusive-v0.4.0 branch at reviewed main can publish; both reusable source validation and installed-package workflows run within the same run. All five installed combinations require all twelve new stages, exact package/payload digest, source/tree/version/run and the existing genuine Windows standard-user proof. No Windows deferral flag is present. Browser component hashes are checked against the publishing checkout, and full Rust/TAP summaries reject skipped, ignored, filtered, truncated or failing tests.

Seventeen synthetic publisher contracts pass locally, covering wrong source/run, missing combinations, old stages, elevated Windows, altered bytes, unsafe names, stale browser sources and a moved main. The unchanged transport suite has20 passing tests. The new parser was also exercised on actual85587 source logs: Windows428 Rust and139 frontend; Ubuntu417 Rust and139 frontend. Those old-revision counts are not claimed as new-source acceptance. The publisher reuses draft-first non-overwriting transport and requires anonymous downloads to match all seven final asset sizes/digests. The native gate still does not itself authorize publication.

Manual impact of new release code is restricted to release orchestration, artifact readers and docs; it grants no new runtime capability. Only the final publish job has contents:write. Source mutation, token rotation, approval and executor code are unchanged in this closure. New guide documents upgrade/offline consent, configurable units, strict refresh reuse, draining/recovery, rollback and the explicit unverified real-ChatGPT/OS-toast boundaries. This remains a pre-release, not a stable all-environments or real-account PASS claim.

Status: awaiting final changed-source CI/native matrix, reviewed merge, final same-run release build, and anonymous published-byte receipt. No asset may be promoted from a failing run.
